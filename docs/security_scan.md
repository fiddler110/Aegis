...
**Secret env allowlisting (P7.2, inverted to an allowlist by P81.26/FIND-26):** The `local` and `os` backends run on the host. They used to inherit the daemon's full environment minus a denylist — which fails open for any secret-bearing variable (`ANTHROPIC_API_KEY`/`OPENAI_API_KEY` included, but also anything else the operator's shell happens to export) that nobody thought to strip; CLAUDE.md is explicit that provider credentials live in that environment by design. Both backends now start from an *empty* environment and pass through only `sandbox.DefaultEnvAllow` (`PATH`, `HOME`, locale, the Go toolchain's own vars, a couple of proxy settings) plus anything named in `sandbox.env_allow`; `sandbox.strip_env` is still applied on top of that allowlisted set as a defensive second layer. The `container` backend forwards a much narrower default (`sandbox.DefaultContainerEnvAllow`: locale and proxy settings only, since `PATH`/`HOME`/`GOPATH` name host filesystem paths that don't exist inside a container) plus `sandbox.env_allow`, via explicit `-e NAME=value` flags — previously it forwarded no host env at all, which was maximally safe but meant a corporate-proxy `HTTPS_PROXY` never reached a containerized build either.
...
If `sandbox.backend` is `container` (the default) or `auto` and no runtime can be initialized (e.g. the Docker daemon isn't running), the daemon cascades to the `os` backend (seatbelt/bwrap) — still real isolation, just not the one named. If `sandbox.backend` is `os` and no OS sandbox mechanism is available, or the cascade above also fails to find one, the only place left to land is the unsandboxed local backend, which is a silent security downgrade if you don't notice it. As of P81.22/FIND-22:

- `sandbox.strict` **defaults to `true`**: landing on unsandboxed local is a hard startup failure, not a silent WARN, on any host with neither a container runtime nor OS-level isolation (every current Windows box, until P77.6, or a macOS/Linux box missing both Docker and seatbelt/bwrap). The error names a concrete next step (install Docker/Podman, or explicitly accept the risk). Set `sandbox.strict: false` only once you've made that choice knowingly.
- Strict guards only the *last* step of the cascade, not the whole thing: a container→os fallback still succeeds under strict, since OS-level isolation is real (if different) containment — it just isn't the exact runtime named.
- Whichever way it resolves, it's always logged at `WARN` (non-strict) or refused with an error (strict).
- The effective backend and any fallback are reported by the authenticated `/status` (`sandbox_backend`, `sandbox_fallback`/`sandbox_fallback_reason`) — surfaced continuously in the TUI sidebar, not just once at the moment a fallback happens — and an execute-capability approval prompt names the effective backend too, so "this command will run unconfined" is visible at the decision, not only in a startup log line. `/status` is deliberately **not** on the unauthenticated `/healthz`: that would tell any local process holding no credential that command isolation had degraded.
