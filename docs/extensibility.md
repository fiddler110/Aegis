...
for how Aegis marks that output's provenance and runs the (on-by-default
since P27.13) `scan_output` heuristic scan, in addition to the capability
gating described below. The same trust boundary applies in the outbound
direction: tool-call arguments are forwarded verbatim to the target server,
and the opt-in `scan_arguments` check (same doc) flags credential-shaped
content in them before they leave the process.
