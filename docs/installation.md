...
3. **[3] Install security scanner tools** *(opt-in — never included in `all`)* — runs `aegis security install <tool> --yes` for every scanner Aegis knows about (opengrep, gosec, bandit, brakeman, njsscan, trivy, gitleaks, kubescape, hadolint, grype, dockle, osv-scanner, syft, nmap, nuclei), using the binary from action 1. Best-effort: several tools need a specific language toolchain (Go/pipx/gem) or package manager (Homebrew/scoop), so an individual failure is reported in a summary rather than aborting the run. Requires action 1 to have run first (or `aegis` already on `PATH`). See [security_scan.md](security_scan.md) for what each tool does and how per-tool installs/container fallbacks work outside the build script.
