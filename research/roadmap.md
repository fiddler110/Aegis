...
test. Full writeup in [releases.md](releases.md#latest-changes). **P27.16** (the other remaining
Tier 3 item, pre-write guard check / quarantine on FAIL) was being implemented in parallel by a
separate agent — see [Priority Order](#priority-order) for its current status.
...
**Open items:** P27.16 (2026-07-13 threat-model finding, Tier 3 — see
[Priority Order](#priority-order)), being implemented in parallel by a separate agent. Tiers 1 and 2
(**P27.1–P27.13**) and Tier 3's **P27.14–P27.15, P27.17–P27.18** shipped 2026-07-13 (P27.18 and
P27.17 both shipped out of order, ahead of P27.16, each fully self-contained). Tier 4 is 5 items —
the pre-existing P25.9/P13.3.3/P6.1 plus **P27.19**/**P27.20** (see
[Parked](#open-work--parked-tier-4)).

**Next session:** confirm **P27.16** (pre-write guard check / quarantine on FAIL) has landed — it
was the last open Tier 3 item and the only one still in flight as of this update. If it has, Tier 3
is fully closed and the next trigger is a new threat-model pass, a reported incident, a new feature
...
**Tier 3** (1 open item — real value, larger or sequence-dependent; **P27.14–P27.15, P27.17–P27.18**
