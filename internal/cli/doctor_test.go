...
// sandbox forced to "local", every scanner disabled) reports no FAIL rows
// and a nil error (main.go maps that to exit 0). The sandbox row itself is
// expected to be a WARN, not a PASS, since P27.14/FIND-04: local gives no
// isolation and doctor now says so rather than passing silently.
//
// sandbox.backend is pinned to "local" via env rather than left at the
// built-in default: that default is "os" (safer, and what a config file
// generated by --first-init also writes explicitly), which this test's
// host may or may not actually be able to serve — a Linux/macOS box with
// bwrap/seatbelt would make it active with no fallback at all (a PASS, not
// this test's WARN), and a host without one would still WARN but with the
// fallback wording, not the "no isolation" wording this test checks for.
// Forcing "local" keeps the FIND-04 assertion below deterministic across
// every CI host regardless of what the default is.
//
// Unlike TestDoctorNamesPodmanMisconfig this needs no selectSandbox seam
// (P34.7): the "local" backend takes SelectSandbox's `case "", "local"`,
