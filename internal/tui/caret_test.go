...
func TestStreamingCaretBlinksAtWriteHead(t *testing.T) {
	...
	m.animStep = caretBlinkPeriod / 2 // blink-off frame
	m.refresh()
	off := plainView(m)
...
// must not render once m.streaming is false, regardless of live text.
func TestStreamingCaretAbsentWhenNotStreaming(t *testing.T) {
	...
	m.animStep = 0 // would be a blink-on frame if streaming were true
	m.refresh()
	got := plainView(m)
...
// phase: m.streaming is true but no tokens have arrived yet (liveText is
