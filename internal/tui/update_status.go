...
func (m model) updateTeammatesUpdate(msg teammatesUpdateMsg) (tea.Model, tea.Cmd) {
	m.sessionMeta.teammates = msg.items
	m.refresh()
	return m, nil
}
...
func (m model) updateStatusInfo(msg statusInfoMsg) (tea.Model, tea.Cmd) {
	...
	if msg.err == nil && msg.info.ContextWindow > 0 {
		m.usage.srvCtxWin = msg.info.ContextWindow
		m.usage.srvCtxWinSrc = msg.info.ContextWindowSource
	}
	...
	if msg.err == nil && msg.info.SandboxBackend != "" {
		m.conn.sandboxBackend = msg.info.SandboxBackend
	}
	// P81.23/FIND-23: same "keep the last known value on a transient error"
	// posture as SandboxBackend above.
	if msg.err == nil {
		m.conn.cronJobCount = msg.info.CronJobCount
		m.conn.cronAutoApproveCount = msg.info.CronAutoApproveCount
		m.conn.cronUnconfirmedCount = msg.info.CronUnconfirmedCount
	}
	...
	m.conn.connKnown = true
	if msg.err != nil {
		m.conn.connReachable, m.conn.connLatencyMS = false, 0
	} else {
		m.conn.connReachable = msg.info.ProviderReachable
		m.conn.connLatencyMS = msg.info.ProviderLatencyMS
	}
	return m, nil
}
