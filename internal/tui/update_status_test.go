...
// listing: a silent ListCronJobs poll result must land in m.conn.cronJobs and
...
// this message at all (m.conn.cronJobs simply stays nil in that case).
func TestUpdateCronJobsPopulatesSidebar(t *testing.T) {
	...
	if len(m.conn.cronJobs) != 1 || m.conn.cronJobs[0].Title != "nightly scan" {
		t.Fatalf("expected m.conn.cronJobs populated with the polled job, got %+v", m.conn.cronJobs)
	}
	got := plainView(m)
