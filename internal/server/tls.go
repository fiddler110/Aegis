...
func ensureTLSCert(certPath, keyPath string) (tls.Certificate, error) {
	...
	// non-inherited ACL on Windows via restrictToOwner (token_windows.go /
	// token_other.go). If a future Windows-ACL-hardening pass (P24.16/
	// FIND-29) generalizes this into a shared fsguard-style helper, this call
	// can switch to it without changing daemon.key's on-disk contents.
