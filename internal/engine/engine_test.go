...
// token's worth of information) must estimate to noticeably more tokens
// than the same rune count of plain ASCII.
func TestEstimateTokensDenseScriptsCostMoreThanASCII(t *testing.T) {
	ascii := "aaaaaaaaaa" // 10 runes, ASCII
	cjk := "一二三四五六七八九十"   // 10 runes, CJK Unified Ideographs

	asciiEst := estimateTokens(ascii)
	cjkEst := estimateTokens(cjk)

	if asciiEst != 3 { // (10+3)/4 = 3
		t.Errorf("estimateTokens(ascii) = %d, want 3", asciiEst)
	}
	if cjkEst != 10 { // one token per dense-script rune
		t.Errorf("estimateTokens(cjk) = %d, want 10", cjkEst)
	}
	if cjkEst <= asciiEst {
...
// TestEstimateTokensOtherScriptsMiddleGround proves non-ASCII, non-dense
// scripts (Cyrillic here) are priced between ASCII and CJK, not lumped in
// with either.
func TestEstimateTokensOtherScriptsMiddleGround(t *testing.T) {
	cyrillic := "привет мир" // 9 Cyrillic letters + 1 ASCII space
	got := estimateTokens(cyrillic)
	if got != 6 { // 9 "other" runes -> (9+1)/2=5, plus 1 ASCII space -> (1+3)/4=1
		t.Errorf("estimateTokens(cyrillic) = %d, want 6", got)
	}
}
