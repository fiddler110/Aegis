...
docs and the verbatim skeletons; the five `.py` files here are **deterministic
...
All five are **Python 3.7+, standard-library only** (no `pyyaml` or other
...
every decision**. `recon.py` lists only symbols that actually exist (so it
structurally cannot invent the abstract components the skill warns against) and
...
```bash
# Compile-check all five (no third-party deps to install)
