...
| `verify.py` | Phase 6 (review round) | Nineteen mechanical cross-file assertions. **Structure (1-15):** no leftover skeleton syntax, component-name consistency, dataflow refs defined, threat↔coverage bijection, finding-id sequence, tier/prerequisite consistency, count agreement, no forbidden coverage statuses, external-AV consistency, architecture↔analysis deployment-classification agreement, coverage-ledger completeness, **finding-bodies non-empty** (a `FIND-##` subsection whose prose body was left blank — the hollow-report case), coverage-table↔per-finding Related-Threats agreement, no data row duplicating its own table header, and **section-bodies non-empty** — the same hollow-body property extended to *every* scaffolded section in all five files (an empty Deployment Classification, Security Infrastructure Inventory, PASTA stage or Executive Summary), asserted against `scaffold.py`'s `.scaffold-manifest.json`. **Substance floor (16-19, P52.8)** — the only checks that judge content, and the only substance gate the TUI path has: an Evidence cell that is a bare filename with no line number/symbol/config key, a required-substantive cell that is a literal `TBD`/`N/A`/`see code`, a table that is *nothing but* "none identified", and a narrative section shorter than one sentence. Deliberately permissive (a false failure costs a verify bounce); every threshold is a `--flag` — see `python verify.py --help`. Built on a generic markdown-table parser (survives column reordering). Prints PASS/FAIL per check. | `python verify.py <run-dir> [--quiet]` |
