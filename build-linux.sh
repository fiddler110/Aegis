...
#   ./build-linux.sh 3                                  # container scanner images only
#   ./build-linux.sh 4                                  # host scanner binaries only
#   ./build-linux.sh all                                # actions 1 + 2 (never 3/4 — opt-in)
#   ./build-linux.sh "all 3"                             # actions 1 + 2 + 3 (recommended)
...
# Detect a runtime that can BUILD an image. Only docker and podman can: the
# build shells out to "<runtime> build -f Containerfile", and Aegis records
# which engine built the image because a locally-built image exists solely in
# that engine's storage.
CONTAINER_RUNTIME=""
...
# Distinguish "not installed" from "installed but the daemon/machine is stopped",
# because the fix is completely different (install vs. `podman machine start`).
...
if [ -n "${CONTAINER_RUNTIME}" ]; then
    ...
    detail "Runtime: ${CONTAINER_RUNTIME_PRESENT_BUT_DOWN} found but not responding — start it first"
    detail "         (dockerd / 'podman machine start'), then re-run with 3"
else
    detail "Runtime: NONE FOUND — install docker or podman, or use action [4] instead"
fi
...
if [ "${RUN_IMAGES}" = true ]; then
    ...
    if [ -z "${AEGIS_BIN}" ]; then
        ...
        if [ -n "${CONTAINER_RUNTIME_PRESENT_BUT_DOWN}" ]; then
            ...
            detail "Start it and re-run with 3:  sudo systemctl start docker   |   podman machine start"
        else
            warn "No container runtime found — the images need docker or podman to build."
            detail "Install one, or use action [4] to install host scanner binaries instead."
        fi
    else
...
if [ "${RUN_IMAGES}" != true ]; then
    detail "./build-linux.sh 3              build the container scanner images (opt-in, not run yet)"
fi
if [ "${RUN_TOOLS}" != true ]; then
    detail "./build-linux.sh 4              install host scanner binaries (opt-in, not run yet)"
fi
if [ "${RUN_IMAGES}" = true ]; then
